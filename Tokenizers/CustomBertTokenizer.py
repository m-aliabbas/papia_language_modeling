...
class CustomBertTokenizer:
    def __init__(self, base_model_path=config['base_model_path'], tokenizer_dir='./tokenizers/',
        ...
        """
        Initialize the CustomBertTokenizer class.

        Args:
            base_model_path (str): Path to the base BERT model.
        ...
        self.tokenizer = BertTokenizerFast.from_pretrained(self.base_model_path)
    ...
    def train_tokenizer(self, cleaned_csv_path, tokenizer_name=''):
        ...
        Returns:
            transformers.PreTrainedTokenizerFast: Trained BERT tokenizer.
        """
        if self.load_data(cleaned_csv_path):
            ...
            bert_tokenizer = self.tokenizer.train_new_from_iterator(text_iterator=self.batch_iterator(),
                                                                    vocab_size=vocab_size,
                                                                    max_length=self.max_length,
                                                                    model_max_length=self.model_max_length)
            bert_tokenizer.save_pretrained(self.tokenizer_dir + tokenizer_name)
...
if __name__ == '__main__':
    custom_bert_tokenizer = CustomBertTokenizer(base_model_path=config['base_model_path'])     
      
    custom_bert_tokenizer.train_tokenizer(cleaned_csv_path=config['cleaned_csv_path'],
                                          tokenizer_name=config['tokenizer_name'])
            
    tokenizer = custom_bert_tokenizer.load_tokenizer(tokenizer_dir=config['tokenizer_dir'],
                                                     tokenizer_name=config['tokenizer_name'])
    print(tokenizer)
